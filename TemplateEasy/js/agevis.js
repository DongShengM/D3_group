...
class AgeVis {
    ...
    wrangleData() {
        ...
        //
        let votesPerAge = d3.range(0,100).map(()=>0)


        // Iterate over each day and accumulate values for each age - use vis.filteredData
        //
        // console.log( vis.filteredData );
        // vis.filteredData.forEach(
        // ...
        // console.log( votesPerAge );

        this.filteredData.forEach((temp)=>{
            temp.ages.forEach((d,i)=>{
                votesPerAge[i]+=d
            })
        })
    ...
    onSelectionChange(selectionStart, selectionEnd) {
        ...
        vis.filteredData = vis.data.filter(function(d){
          return  selectionStart<= d.time&& d.time<=selectionEnd
        })
        // ...
