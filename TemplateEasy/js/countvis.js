...
class CountVis {
	...
	initVis() {
		...
		// Set domains
		let minMaxY = [0, d3.max(vis.data.map(function (d) {
			return d.count;
		}))];
		vis.y.domain(minMaxY);

		let minMaxX = d3.extent(vis.data.map(function (d) {
			return d.time;
		}));
		...
		// *** TO-DO ***
		//
		vis.xOrig = vis.x.copy(); // save original scale
		//
		...
		vis.zoomFunction = function (event) {
			...
			vis.x.domain(xScaleModified.domain())
	...
	updateVis(event) {
		...
		let isBrush =d3.select('.selection').style('display')
